...
variable {C : Type u₁} [Quiver.{v₁} C] {D : Type u₂} [Category.{v₂} D]
...
structure NatTransq (F G : C ⥤q D) : Type max u₁ v₂ where
...
lemma NatTransq.naturality' {F G : C ⥤q D} (self : NatTransq G F) ⦃X Y : C⦄ (f : Y ⟶ X) :
...
attribute [reassoc (attr := simp)] NatTransq.naturality

attribute [grind _=_] NatTransq.naturality

@[to_dual self]
theorem congr_appq {F G : C ⥤q D} {α β : NatTransq F G} (h : α = β) (X : C) : α.app X = β.app X := by
  cat_disch

namespace NatTransq

/-- `NatTransq.id F` is the identity natural transformation on a functor `F`. -/
protected def id (F : C ⥤q D) : NatTransq F F where app X := 𝟙 (F.obj X)

@[simp]
theorem id_app' (F : C ⥤q D) (X : C) : (NatTransq.id F).app X = 𝟙 (F.obj X) := rfl

instance (F : C ⥤q D) : Inhabited (NatTransq F F) := ⟨NatTransq.id F⟩
...
variable {F G H : C ⥤q D}

/-- `vcomp α β` is the vertical compositions of natural transformations. -/
@[to_dual self (reorder := F H, α β)]
def vcomp (α : NatTransq F G) (β : NatTransq G H) : NatTransq F H where
...
@[to_dual self]
theorem vcomp_app (α : NatTransq F G) (β : NatTransq G H) (X : C) :
...
commutes.
-/
example {F G : C ⥤q D} (α : NatTransq F G) {X Y U V : C} (f : X ⟶ Y) (g : Y ⟶ U) (h : U ⟶ V) :
