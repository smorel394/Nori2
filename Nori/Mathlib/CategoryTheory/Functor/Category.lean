...
public import Nori.Mathlib.CategoryTheory.NatTrans
...
open NatTransq Category CategoryTheory.Functor

variable (C : Type u₁) [Quiver.{v₁ + 1} C] (D : Type u₂) [Category.{v₂} D]

attribute [local simp] vcomp_app

variable {C D} {E : Type u₃} [Category.{v₃} E]
variable {E' : Type u₄} [Category.{v₄} E']
variable {F G H I : C ⥤q D}

instance Functor.quiver : Quiver (C ⥤q D) where
  Hom F G := NatTransq F G

attribute [local grind =] NatTransq.id_app' in
...
this is a small category at the next higher level.
-/
instance Prefunctor.category : Category.{max u₁ v₂} (C ⥤q D) where
  Hom F G := NatTransq F G
  id F := NatTransq.id F
  comp α β := vcomp α β
...
@[ext, grind ext]
theorem ext' {α β : F ⟶ G} (w : α.app = β.app) : α = β := NatTransq.ext w
...
@[simp, grind =]
theorem id_app (F : C ⥤q D) (X : C) : (𝟙 F : F ⟶ F).app X = 𝟙 (F.obj X) := rfl

@[simp, grind _=_]
theorem comp_app {F G H : C ⥤q D} (α : F ⟶ G) (β : G ⟶ H) (X : C) :
    (α ≫ β).app X = α.app X ≫ β.app X := rfl

attribute [reassoc] comp_app

@[reassoc]
theorem app_naturality {F G : C ⥤q (D ⥤q E)} (T : F ⟶ G) (X : C) {Y Z : D} (f : Y ⟶ Z) :
    (F.obj X).map f ≫ (T.app X).app Z = (T.app X).app Y ≫ (G.obj X).map f :=
  (T.app X).naturality f

@[reassoc (attr := simp)]
theorem naturality_app {F G : C ⥤q (D ⥤q E)} (T : F ⟶ G) (Z : D) {X Y : C} (f : X ⟶ Y) :
    (F.map f).app Z ≫ (T.app Y).app Z = (T.app X).app Z ≫ (G.map f).app Z :=
  congr_fun (congr_arg app (T.naturality f)) Z

@[reassoc]
theorem naturality_app_app {F G : C ⥤q (D ⥤q (E ⥤q E'))}
    (α : F ⟶ G) {X₁ Y₁ : C} (f : X₁ ⟶ Y₁) (X₂ : D) (X₃ : E) :
    ((F.map f).app X₂).app X₃ ≫ ((α.app Y₁).app X₂).app X₃ =
      ((α.app X₁).app X₂).app X₃ ≫ ((G.map f).app X₂).app X₃ :=
  congr_app (NatTransq.naturality_app α X₂ f) X₃
