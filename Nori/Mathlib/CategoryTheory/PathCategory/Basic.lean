...
public import Nori.Mathlib.CategoryTheory.NatIso
...
@[simp]
def liftNatTransq {F G : V ⥤q C} (u : F ⟶ G) : lift F ⟶ lift G where
  app := u.app
  naturality := by
    apply Paths.induction (P := fun {X Y} f ↦ (lift F).map f ≫ u.app Y = u.app X ≫ (lift G).map f)
    · intro _
      change 𝟙 _ ≫ _ = _ ≫ 𝟙 _
      simp
    · intro _ _ _ f g eq
      dsimp at eq ⊢
      simp only [Functor.map_comp, lift_toPath, Category.assoc, NatTransq.naturality]
      rw [← assoc, eq, assoc]

variable (C)

def liftFunctor : (V ⥤q C) ⥤ (Paths V ⥤ C) where
  obj F := lift F
  map u := liftNatTransq u
--  map_id F := by aesop_cat
--  map_comp u v := by aesop_cat

def liftInverse : (Paths V ⥤ C) ⥤ (V ⥤q C) where
  obj G := Paths.of V ⋙q G.toPrefunctor
  map u := {app := u.app}
--  map_id := by aesop_cat
--  map_comp := by aesop_cat

def liftEquivalence : (Paths V ⥤ C) ≌ (V ⥤q C) where
  functor := liftInverse C
  inverse := liftFunctor C
  unitIso := by
    refine NatIso.ofComponents (fun F ↦ ?_) (fun u ↦ ?_)
    · dsimp [liftFunctor, liftInverse]
      refine NatIso.ofComponents (fun _ ↦ Iso.refl _) (fun f ↦ ?_)
      dsimp
      rw [id_comp, comp_id]
      apply Paths.induction (P := fun f ↦ F.map f = (lift (of V ⋙q F.toPrefunctor)).map f)
      · intro _
        dsimp
      ...
      · intro _ _ _ f g eq
        dsimp at eq ⊢
        rw [F.map_comp, eq, Functor.map_comp]
        congr
  ...
  counitIso := by
    ...
    · refine NatIsoq.ofComponents (fun _ ↦ Iso.refl _) ?_
      intro _ _ f
      dsimp [liftFunctor, liftInverse]
      simp
    . intro _ _ u
