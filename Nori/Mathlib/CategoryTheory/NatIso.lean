...
public import Nori.Mathlib.CategoryTheory.Functor.Category
...
variable {C : Type u₁} [Quiver.{v₁ + 1} C] {D : Type u₂} [Category.{v₂} D] {E : Type u₃}
...
@[simps (attr := grind =)]
def app {F G : C ⥤q D} (α : F ≅ G) (X : C) :
    F.obj X ≅ G.obj X where
  hom := α.hom.app X
  inv := α.inv.app X

@[reassoc (attr := simp), grind =]
theorem hom_inv_id_app {F G : C ⥤q D} (α : F ≅ G) (X : C) :
    α.hom.app X ≫ α.inv.app X = 𝟙 (F.obj X) := by cat_disch

@[reassoc (attr := simp), grind =]
theorem inv_hom_id_app {F G : C ⥤q D} (α : F ≅ G) (X : C) :
    α.inv.app X ≫ α.hom.app X = 𝟙 (G.obj X) := by cat_disch

@[reassoc (attr := simp)]
lemma hom_inv_id_app_app {F G : C ⥤q (D ⥤ E)} (e : F ≅ G) (X₁ : C) (X₂ : D) :
    (e.hom.app X₁).app X₂ ≫ (e.inv.app X₁).app X₂ = 𝟙 _ := by cat_disch

@[reassoc (attr := simp)]
lemma inv_hom_id_app_app {F G : C ⥤q (D ⥤ E)} (e : F ≅ G) (X₁ : C) (X₂ : D) :
    (e.inv.app X₁).app X₂ ≫ (e.hom.app X₁).app X₂ = 𝟙 _ := by cat_disch

@[reassoc (attr := simp)]
lemma hom_inv_id_app_app_app {F G : C ⥤q (D ⥤ (E ⥤ E'))} (e : F ≅ G)
    (X₁ : C) (X₂ : D) (X₃ : E) :
    ((e.hom.app X₁).app X₂).app X₃ ≫ ((e.inv.app X₁).app X₂).app X₃ = 𝟙 _ := by cat_disch

@[reassoc (attr := simp)]
lemma inv_hom_id_app_app_app {F G : C ⥤q (D ⥤ (E ⥤ E'))} (e : F ≅ G)
    (X₁ : C) (X₂ : D) (X₃ : E) :
...
@[simp]
theorem trans_app {F G H : C ⥤q D} (α : F ≅ G) (β : G ≅ H) (X : C) :
    Isoq.app (α ≪≫ β) X = Isoq.app α X ≪≫ Isoq.app β X :=
  rfl
...
@[simp]
theorem inv_inv_app {F G : C ⥤q D} (e : F ≅ G) (X : C) : inv (e.inv.app X) = e.hom.app X := by
  cat_disch
...
/-
@[simp]
theorem inv_map_inv_app (F : C ⥤q (D ⥤ E)) {X Y : C} (e : X ≅ Y) (Z : D) :
...
theorem ofComponents.app (app' : ∀ X : C, F.obj X ≅ G.obj X) (naturality) (X) :
    Isoq.app (ofComponents app' naturality) X = app' X := by cat_disch
...
@[simps]
def hcomp {F G : C ⥤q D} {H I : D ⥤ E} (α : F ≅ G) (β : H ≅ I) : F ⋙ H ≅ G ⋙ I where
  hom := α.hom ◫ β.hom
  inv := α.inv ◫ β.inv
-/

theorem isIso_map_iff {F₁ F₂ : C ⥤q D} (e : F₁ ≅ F₂) {X Y : C} (f : X ⟶ Y) :
    IsIso (F₁.map f) ↔ IsIso (F₂.map f) := by
  revert F₁ F₂
  suffices ∀ {F₁ F₂ : C ⥤q D} (_ : F₁ ≅ F₂) (_ : IsIso (F₁.map f)), IsIso (F₂.map f) from
...
lemma NatTransq.isIso_iff_isIso_app {F G : C ⥤q D} (τ : F ⟶ G) :
    IsIso τ ↔ ∀ X, IsIso (τ.app X) :=
  ⟨fun _ ↦ inferInstance, fun _ ↦ NatIsoq.isIso_of_isIso_app _⟩

namespace Prefunctor

variable (F : C ⥤q D) (obj : C → D) (e : ∀ X, F.obj X ≅ obj X)
...
@[simps obj]
def copyObj : C ⥤q D where
  obj := obj
  map f := (e _).inv ≫ F.map f ≫ (e _).hom

/-- The functor constructed with `copyObj` is isomorphic to the given functor. -/
@[simps!]
def isoCopyObj : F ≅ Prefunctor.copyObj F obj e :=
  NatIsoq.ofComponents e (by simp [Prefunctor.copyObj])

end Prefunctor

/-
@[reassoc]
lemma NatTransq.naturality_1 {F G : C ⥤q D} (α : F ⟶ G) {X Y : C} (e : X ≅ Y) :
